...
import { parseUpdatedManga, generateSearch, parseChapterDetails, isLastPage, parseTags, parseChapters, parseHomeSections, parseMangaDetails, parseViewMore, UpdatedManga } from "./AsuraScanParser"

const DOMAIN = 'https://asura-scans.com/'
const method = 'GET'

export const AsuraScanInfo: SourceInfo = {
  version: '1.0.11',
  name: 'AsuraScan',
  icon: 'icon.png',
  author: 'Netsky',
  authorWebsite: 'https://github.com/TheNetsky',
  description: 'Extension that pulls manga from AsuraScan.',
  hentaiSource: false,
...
export class AsuraScan extends Source {
  getMangaShareUrl(mangaId: string): string | null { return `${DOMAIN}/manga/${mangaId}` }
  ...
  async filterUpdatedManga(mangaUpdatesFoundCallback: (updates: MangaUpdates) => void, time: Date, ids: string[]): Promise<void> {
    ...
    while (updatedManga.loadMore) {
      const request = createRequestObject({
        url: `${DOMAIN}/latest-releases/${page++}`,
        method,
      });
  ...
  async searchRequest(query: SearchRequest): Promise<PagedResults> {
    ...
    for (const m of data.manga) {
      ...
      const image: string = DOMAIN + m.image;
